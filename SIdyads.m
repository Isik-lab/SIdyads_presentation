...
if ~exist(topout, 'dir')
    ...
    n_extra_TRs = 1;
    n_extra_TR_trials = 10;
    ending_wait_time = 9*TR;
    start_wait_time = 2*TR;
    ...
    temp = dir(fullfile('social_dyad_videos_500ms','crowd_videos','*.mp4'));
    ...
    for itrial = 1:n_trials
        ...
        if T.condition(itrial) == 1
            T.movie_path{itrial} = fullfile(curr, 'social_dyad_videos_500ms','videos_mov',video_name);
        elseif T.condition(itrial) == 0
            T.movie_path{itrial} = fullfile(curr, 'social_dyad_videos_500ms','crowd_videos_mov',video_name);
        end
    end
